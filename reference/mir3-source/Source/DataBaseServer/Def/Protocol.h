...
//For InterServer
#define    ISM_PASSWDSUCCESS        100  //패스워드 통과, Certification+ID
#define    ISM_CANCELADMISSION      101  //Certification 승인취소..
#define    ISM_USERCLOSED           102  //사용자 접속 끊음
#define    ISM_USERCOUNT            103  //이 서버의 사용자 수
#define    ISM_TOTALUSERCOUNT       104
#define    ISM_SHIFTVENTURESERVER   110
...
typedef struct tag_ADDUSERENTRYINFO
{
	char		szQuiz2[21];
	char		szAnswer2[13];
	char		szBirthDay[11];
	char		szMobilePhone[14];
	char		szMemo1[21];
	char		szMemo2[21];
}_ADDUSERENTRYINFO, *_LPADDUSERENTRYINFO;
