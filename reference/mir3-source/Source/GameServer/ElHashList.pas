...
{ Modified 29.10.99 20:53:13 by the CDK, Version 3.02 Rev. F }
...
type EElHashListError = class (Exception)
     end;

const MaxHashListSize = Maxint div 16;
//const MaxHashListSize = Maxint div 65536;
...
{$ENDIF}
  ...
  public
    ...
    procedure Add( Hash : string; Value : Pointer );
    procedure Delete( Hash : string );
    procedure Insert( Index : integer; Hash : string; Value : pointer );
    ...
    property Items[Index : integer ] : pointer read GetByIndex;  { Public }
...
procedure TElHashList.Add( Hash : string; Value : Pointer );  { public }
begin
  Insert(FCount, Hash, Value);
end;  { AddItem }

procedure TElHashList.Delete( Hash : string );  { public }
var index : integer;
begin
  Index:=GetIndex(Hash);
...
procedure TElHashList.Insert( Index : integer; Hash : string; Value : pointer );  { public }
var P : PHashRecord;
    MD5 : TCrMD5;
    b : boolean;
    i : integer;
...
begin
  ...
  case FHashType of
    ...
    ehtCRC32: P^.Hash:=pointer(CrcStr(Hash));
    ehtMD5:
      begin
        New(P^.Hash);
        MD5:=TCrMD5.Create;
  ...
  begin
    if FQuickHash
       then h:=CalcQuickHash(Hash)
       else h:=CrcStr(Hash);
    while (i<FCount) and (integer(FList^[i]^.Hash)<>h) do inc(i);
  end
  else
  begin
    while (i<FCount) and
          (not CompareMem(P.Hash,FList^[i]^.Hash,SizeOf(Integer)*4)) do inc(i);
  end;
  b := i <> FCount;
  if b then
  begin
...
begin
  ...
  if i=-1 then
  begin
    if RaiseOnAbsence then raise EElHashListError.Create('Hash is absent.') else result:=nil;
  end else result:=FList^[i]^.ItemData;
end;  { GetItem }
...
begin
  ...
  if i=FCount then
  begin
    if RaiseOnAbsence then raise EElHashListError.Create('Hash is absent.') else result:=-1;
  end else result:=i;
end;  { GetIndex }
...
begin
  if (Index<0) or (Index>=FCount)
    then raise EElHashListError.Create('Invalid index.')
    else result:=FList^[Index]^.ItemData;
end;  { GetByIndex }
...
begin
  result:=0;
  for i:=1 to Length(Hash) do result:=result*5 + byte(Hash[i]);
end;  { CalcQuickHash }
