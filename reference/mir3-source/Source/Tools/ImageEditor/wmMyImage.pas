...
interface
uses
  Windows, Classes, Graphics, SysUtils, MyDirect3D9, TDX9Textures, HUtil32, WIL, DES{, ZLibEx};
...
type
  ...
  private
    ...
    procedure LoadIndex();
    function CopyImageDataToTexture(Buffer: PChar; Texture: TDXImageTexture; Width, Height: Word; bitLength: Byte): Boolean;
      ...
      if FFileStream.Read(Buffer^, ReadSize) = ReadSize then begin
        ...
        if pDXTexture.Surface <> nil then begin
          if not CopyImageDataToTexture(outBuffer, pDXTexture.Surface, imginfo.DXInfo.nWidth, imgInfo.DXInfo.nHeight,
            GetFormatBitLen(imginfo.btImageFormat)) then begin
...
function TWMMyImageImages.CopyImageDataToTexture(Buffer: PChar; Texture: TDXImageTexture; Width, Height: Word; bitLength: Byte): Boolean;
var
  X, Y: Integer;
  nColorId:LongWord;
  wCol16:Word;
  ...
  if Texture.Lock(lfWriteOnly, Access) then begin
    try
      ...
      if (Texture.Size.X = Width) and (Texture.Size.Y = Height) then begin
        if FLastColorFormat in [WILFMT_R5G6B5] then begin
          nColorId:= 0;
  ...
  if index < FIndexList.Count then begin
    ...
    try
      if FFileStream.Read(Buffer^, ReadSize) = ReadSize then begin
        ...
        Texture.Active := True;
        Result := CopyImageDataToTexture(outBuffer, Texture, imginfo.DXInfo.nWidth, imgInfo.DXInfo.nHeight, GetFormatBitLen(imginfo.btImageFormat));
          ...
          for Y := 0 to Result.Height - 1 do begin
            ...
            for X := 0 to Result.Width - 1 do begin
              case imginfo.btImageFormat of
                ...
                WILFMT_R5G6B5: begin
                  //洋红63519透明色
//                  Result.TransparentColor := g_OutBackColor;
//                  Result.Transparent := True;
                  TColor2RGB(g_OutBackColor, nR, nG, nB);
                  if wReadBuffer^ = 63519 then wWriteBuffer^ := RGB16(nR, nG, nB)//63519
//                  else if wReadBuffer^ = 0 then wWriteBuffer^ := RGB16(8, 0, 0)//63519
