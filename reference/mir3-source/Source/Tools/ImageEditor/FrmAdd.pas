...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, TeCanvas, ExtCtrls, Spin, Gauges, Mask, RzEdit, RzStatus, RzPrgres, wmMyImage, MyD3DX9;
...
type
  ...
  TFormAdd = class(TForm)
    ...
    procedure File_RefreshClick(Sender: TObject);
  private
    procedure ChangeControl(boBegin: Boolean = False);
    function LoadFileToBmp(sFileName: string): TBitmap;
    function FormatBitmap(const Bitmap: TBitmap; boAlpha, boCut: Boolean; BColor: TColor; var Data: PChar;
...
begin
  ...
  File_Refresh.Enabled := not boBegin;
  ...
  if NoneOption.Visible then begin
    ...
    Image_Format.Enabled := (not boBegin) and (not Image_Offset.Checked) and (not Image_Window.Checked);
    ...
    Image_TColor.Enabled := (not boBegin) and (not Image_Offset.Checked) and (Image_Format.ItemIndex <> 3);
    if Image_Format.ItemIndex = 2 then Image_TColor.SelectedColor := clFuchsia
    else Image_TColor.SelectedColor := clBlack;
      ...
      for I := 0 to OpenDialog.Files.Count - 1 do begin
        ...
        if FileExists(FIdxFile) then begin
          ...
          if TempList.Count > 1 then begin
            ...
            FileListInfo.nsPx := StrToIntDef(Templist[2], 0);
            FileListInfo.nsPy := StrToIntDef(Templist[3], 0);
            FileListInfo.nShadow := StrToIntDef(Templist[4], 0);
            if TempList.Count > 5 then
              FileListInfo.btFormat := StrToIntDef(Templist[5], 0);
          end;
        end;
...
var
  i, X, Y, nX, nY, Shadow, ShadowX, ShadowY: Integer;
  StartInt, EndInt, StartPos, EndPos: Integer;
  TempStr, sX, sY, sShadow, sShadowX, sShadowY: string;
...
begin
  ...
  TempStr := GetValidStr3(TempStr, sShadowX, [' ', ',']);
  TempStr := GetValidStr3(TempStr, sShadowY, [' ', ',']);
  TempStr := GetValidStr3(TempStr, sShadow, [' ', ',']);
  X := StrToIntDef(sX, 0);
  Y := StrToIntDef(sY, 0);
  ShadowX := StrToIntDef(sShadowX, 0);
  ShadowY := StrToIntDef(sShadowY, 0);
  Shadow := StrToIntDef(sShadow, 0);
  btFormat := 0;
  ...
  if ImageStream.Checked and Image_Offset.Checked and Image_Offset_App.Checked then begin
    ...
    sX := Trim(sX);
    sY := Trim(sY);
    sShadowX := Trim(sShadowX);
    sShadowY := Trim(sShadowY);
    sShadow := Trim(sShadow);
    if (sX = '') or (sY = '') or (sShadow = '') or (sShadowX = '') or (sShadowY = '') then begin
    ...
    if (btXS = 3) and (btYS = 3) then begin
      for I := StartInt to EndInt do begin
        WMImages.UpdateImageXY(I, X, Y, Shadow, ShadowX, ShadowY);
      end;
    end
    else begin
      for I := StartInt to EndInt do begin
        ...
        WMImages.UpdateImageXY(I, nX, nY, Shadow, ShadowX, ShadowY);
      end;
    end;
    ...
    for I := 0 to File_List.Items.Count - 1 do begin
      ...
      if ImageStream.Checked then begin
        if Image_Offset.Checked then begin //更新坐标
          ...
          WMImages.UpdateImageXY(StartInt + I, X, Y, Shadow, ShadowX, ShadowY);
        end
        else begin //更新图片和坐标
          if Image_ImgAndOffset.Checked and Image_Offset_File.Checked then begin
            X := FileListInfo.nPx;
            Y := FileListInfo.nPy;
            ShadowX := FileListInfo.nsPx;
            ShadowY := FileListInfo.nsPy;
            Shadow := FileListInfo.nShadow;
          ...
          if (Bitmap.Width < MINIMAGESIZE) or (Bitmap.Height < MINIMAGESIZE) or (Bitmap.Width > MAXIMAGESIZE) or (Bitmap.Height > MAXIMAGESIZE) then begin
            ...
            //将图型数据转换为存储数据
            DataBufferLen := FormatBitmap(Bitmap, Image_Alpha.Checked, Image_Cut.Checked,
            ...
            if Image_ImgAndOffset.Checked then begin
              ...
              ImageInfo.DXInfo.shShadowPX := ShadowX;
              ImageInfo.DXInfo.shShadowPY := ShadowY;
              ImageInfo.DXInfo.bShadow := Shadow;
            end
            else begin
...
procedure TFormAdd.File_RefreshClick(Sender: TObject);
var
  sStr: string;
  sr: TSearchRec;
  I: Integer;
...
begin
  ...
  if sBrowseForFolder <> '' then begin
    sStr := sBrowseForFolder;
    if RightStr(sStr, 1) <> '\' then
      sStr := sStr + '\';
    ChangeControl(True);
    File_List.Visible := False;
  ...
  end else if sBrowseForAllFolder <> '' then begin
    sStr := sBrowseForAllFolder;
    if RightStr(sStr, 1) <> '\' then
      sStr := sStr + '\';
    ChangeControl(True);
    File_List.Visible := False;
...
begin
  ...
  try
    ...
    if (ChrBuff[0] + ChrBuff[1]) = 'BM' then begin //BMP
      ...
      Result.LoadFromStream(FileStream);
//    end else begin
//      D3DXCreateTextureFromFile(MyDevice, )
    end;
...
function TFormAdd.FormatBitmap(const Bitmap: TBitmap; boAlpha, boCut: Boolean; BColor: TColor;
...
//    CompressBuf(Buffer, nBuffLen, Pointer(Data), Result);
...
begin
  ...
  if Image_Format.ItemIndex = -1 then Image_Format.ItemIndex := 2;
  ChangeControl;
  ProgressBar.Position := 0;
  ShowModal;
end;
