...
type
  TCrypToSeed = record
    CrypByte: array[0..3] of Byte;
    CrypLong: LongWord;
  end;
  PTCrypToSeed = ^TCrypToSeed;
   ...
   function Decrypt(FName: string): TStringList;
...
function Decrypt(FName: string): TStringList;
var
  ptmp, Mir3Size, CheckSun, CrcSun, I, J: DWORD;
  CrypToSeed: TCrypToSeed;
  DatCrypToSeed: PTCrypToSeed;
  nFileMem, TestMem: TMemoryStream;
  MirRes: Pointer;
  ProcLen: Integer;
begin
  nFileMem := nil;
  ...
  nFileMem := TMemoryStream.Create;
  TestMem := TMemoryStream.Create;
  try
    nFileMem.LoadFromFile(FName);
    Mir3Size := nFileMem.Size;
    if Mir3Size < 8 then Exit;
    MirRes := nFileMem.Memory;
    ptmp := DWORD(MirRes);
    CrypToSeed.CrypByte[0] := $F0;
    CrypToSeed.CrypByte[1] := $39;
    CrypToSeed.CrypByte[2] := $AB;
    CrypToSeed.CrypByte[3] := $8E;
    CrypToSeed.CrypLong := $9FDE1A93;
    DatCrypToSeed := PTCrypToSeed(MirRes);
    CheckSun := 0;
    ProcLen := MakeLong(
      MakeWord(CrypToSeed.CrypByte[3] xor DatCrypToSeed.CrypByte[3],
               CrypToSeed.CrypByte[2] xor DatCrypToSeed.CrypByte[2]),
      MakeWord(CrypToSeed.CrypByte[1] xor DatCrypToSeed.CrypByte[1],
               CrypToSeed.CrypByte[0] xor DatCrypToSeed.CrypByte[0]));

    if ProcLen > 0 then begin
      for I := 0 to (ProcLen - 1) do begin
        CheckSun := CheckSun + (PBYTE(ptmp + 8 + I)^ + 1) * I;
      end;
      CheckSun := CheckSun xor CrypToSeed.CrypLong;
      CrcSun := PDWORD(DWORD(ptmp) + 4)^;
      if CrcSun = CheckSun then begin
        for J := 0 to 3 do begin
          CrcSun := PByte(ptmp + 3 - J)^;
          for I := 0 to (ProcLen - 1) do begin
            PByte(ptmp + 8 + I)^ := PByte(ptmp + 8 + I)^ xor CrcSun;
            CrcSun := CrcSun + 1;
          end;
        end;
        TestMem.SetSize(ProcLen);
        Move(PByte(ptmp + 8)^, TestMem.Memory^, ProcLen);
      end;
    end;
    TestMem.Position := 0;
    Result.LoadFromStream(TestMem);
  finally
    if nFileMem <> nil then FreeAndNil(nFileMem);
    if TestMem <> nil then FreeAndNil(TestMem);
  end;
end;
