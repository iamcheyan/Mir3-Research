...
void CDatabase::DiagRec( int nHandleType, SQLHANDLE hHandle )
{
	...
	while ( true )
	{
		nResult = SQLGetDiagRec( nHandleType, 
								 hHandle, 
								 nRecCnt++, 
								 (byte *) szState, 
								 (long *) &nNativeError, 
...
bool CConnection::SetAttribute( int nAttr, int nValue )
{
	int nResult;

	nResult = SQLSetConnectAttr( m_hDBConn, nAttr, (void *) nValue, sizeof( nValue ) );
	if ( nResult != SQL_SUCCESS )
	{
		CDatabase::DiagRec( SQL_HANDLE_DBC, m_hDBConn );
		return false;
...
{
	return SetAttribute( SQL_ATTR_AUTOCOMMIT, SQL_AUTOCOMMIT_OFF );
}


bool CConnection::EndTran( bool bCommit )
{
	int nResult;

	nResult = SQLEndTran( SQL_HANDLE_DBC, m_hDBConn, bCommit ? SQL_COMMIT : SQL_ROLLBACK );
	if ( nResult != SQL_SUCCESS )
		CDatabase::DiagRec( SQL_HANDLE_DBC, m_hDBConn );

	return SetAttribute( SQL_ATTR_AUTOCOMMIT, SQL_AUTOCOMMIT_ON );
}
...
bool CRecordset::Execute( char *pQuery )
{
	...
	nResult = SQLExecDirect( m_hStmt, (byte *) pQuery, SQL_NTS );
	if ( nResult != SQL_SUCCESS )
	{
		CDatabase::DiagRec( SQL_HANDLE_STMT, m_hStmt );
	...
	{
		nResult = SQLDescribeCol( m_hStmt, 
								  nCount + 1, 
