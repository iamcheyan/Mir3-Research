...
type
  ...
  TColorEffect = (ceNone, ceGrayScale, ceBright, ceRed, ceGreen, ceBlue, ceYellow, ceFuchsia, zdy1, zdy2, zdy3, zdy4, zdy5, zdy6, zdy7, zdy8, zdy9, zdy10, zdy11, zdy12, zdy13, zdy14, zdy15);

  TDataType = (dtAll, dtMusic, dtData, dtMP3, dtWav);

  TLibType = (ltLoadBmp, ltUseCache, ltFileData, ltLoadMemory);
...
{$ENDIF}
  ...
  public
    ...
    function GetCachedImageEx(index: Integer; var px, py, spx, spy, stype: Integer): TDXImageTexture;
...
uses
  wmM2Def, wmM3Def, wmM2Zip, wmM3Zip, wmMyImage;
...
begin
  ...
  FImageCount := 0;
  FFileName := '';
  FFFWWW := ExtractFilePath(ParamStr(0));
  FReadOnly := True;
...
begin
  if FDxTextureArr <> nil then
    for i := 0 to High(FDxTextureArr) do
    begin
      if (FDxTextureArr[i].Surface <> nil) and (GetTickCount - FDxTextureArr[i].dwLatestTime > FFreeSurfaceTick) then
      begin
    ...
    if (FDxTextureArr[index].Surface = nil) and (not FDxTextureArr[index].boNotRead) then begin
      try
        ...
        if FDxTextureArr[index].Surface <> nil then
          Inc(FSurfaceCount);
//        DebugOutStr(FFileName + ' FSurfaceCount: ' + IntToStr(FSurfaceCount));
      except
...
function TWMBaseImages.GetCachedImageEx(index: Integer; var px, py, spx, spy, stype: Integer): TDXImageTexture;
begin
  Result := nil;
  if (index < 0) or (index >= FImageCount) or (FLibType <> ltUseCache) or (not FInitialize) then
    exit;
...
//        DebugOutStr(FFileName + ' FSurfaceCount: ' + IntToStr(FSurfaceCount));
    ...
    px := FDxTextureArr[index].nPx;
    py := FDxTextureArr[index].nPy;
    spx := FDxTextureArr[index].nsPx;
    spy := FDxTextureArr[index].nsPy;
    stype := FDxTextureArr[index].nShadow;
    FDxTextureArr[index].dwLatestTime := GetTickCount;
  end;
  if AutoFreeMemorys and (GetTickCount > FAutoFreeMemorysTime) then begin
    FAutoFreeMemorysTime := GetTickCount + FAutoFreeMemorysTick;
