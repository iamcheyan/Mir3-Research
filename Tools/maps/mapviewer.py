...
<head>
    ...
    <style>
        ...
        .msel-item .msel-un { margin-left:auto; font-size:10px; color:#ff8f6b; background:rgba(72,36,24,.45);
            border:1px solid #5a3a30; border-radius:3px; padding:0 4px; flex-shrink:0; }
        .msel-item.unused { color:#8a8a98; }
...
<body class="wu-shell">
    ...
    <script>
        ...
        // 悬停关闭一律用 document mousemove 探测：drawRoutes() 重建 SVG 会销毁原
        // circle，mouseout 不会触发（tooltip 与 detail 弹窗同病），必须按坐标实时判定。
        document.addEventListener("mousemove", (e) => {
            if (portTooltip.style.display !== "none" &&
                !portTooltip.contains(e.target)) {
                const t = e.target.closest && e.target.closest("circle.port");
                if (!t || !t.dataset.dstmap) portTooltip.style.display = "none";
            }
            if (portDetail.style.display === "none") return;
        ...
        portDetail.addEventListener("click", (e) => {
            ...
            if (dst && maps.some(m => m.name === dst + ".map")) {
                hidePortDetail();
                portTooltip.style.display = "none";   // 跳转换图后原悬停标记被销毁, mouseout 不会再触发
        ...
        routeSvg.addEventListener("click", (e) => {
            ...
            if (portSelKey && portSelKey === c.dataset.key) {
                ...
                hidePortDetail();
                portTooltip.style.display = "none";   // 同上: 跳转时显式关掉悬停小地图
...
<div class="row"><span class="k">动画格</span><span class="v">${cat.animated_cells || 0}</span></div>`;
        ...
        function mselOpen() {
            mselPop.hidden = false;   // 保留上次分类/搜索词 (init 从 localStorage 恢复)
            renderMselTabs(); renderMselList();
            mselList.scrollTop = mselScrollPos;   // renderMselList 内 scrollIntoView 之后再归位
            mselFilter.focus();
            ensureNpcSet().then(() => { renderMselList(); mselList.scrollTop = mselScrollPos; });   // NPC 小点懒加载完成后重绘并归位
        }
        function mselClose() { mselPop.hidden = true; }
        let mselCat = "all";                       // 分类标签筛选（all/town/cave/room/other/unused）
        let mselScrollPos = 0;                     // 列表滚动位置（持久化用）
        let _mselSaveT = null;
        function saveMselState() {                 // 防抖合并进 localStorage 状态
            clearTimeout(_mselSaveT);
            _mselSaveT = setTimeout(() => {
                try {
                    const st = loadState() || {};
                    st.mselCat = mselCat; st.mselQ = mselFilter.value; st.mselScroll = mselScrollPos;
        ...
        function mselFiltered() {
            ...
            const hit = m => !q || m.name.toLowerCase().includes(q) || (m.cn || "").toLowerCase().includes(q);
            return maps.filter(m => {
                const un = m.used === false;   // 服务器 MapInfo 未收录
                if (mselCat === "all") return hit(m);
                if (mselCat === "unused") return un && hit(m);
                return !un && (m.cat || "other") === mselCat && hit(m);
            });
        }
        ...
        const mselItem = (m) => {
            ...
            const un = m.used === false
                ? '<span class="msel-un" title="玩家从出生点经传送门/NPC对话不可达">未使用</span>' : "";
            return '<div class="msel-item' + (m.used === false ? " unused" : "") + '" data-name="' + m.name.replace(/"/g, "&quot;") + '">' +
                dot + '<span class="msel-cn">' + (m.cn || "") + '</span><span>' + m.name + '</span>' + un + '</div>';
        };
        function renderMselTabs() {
            const tabs = document.getElementById("msel-tabs");
            if (!tabs) return;
            const counts = { all: maps.length, unused: 0 };
            for (const m of maps) {
                if (m.used === false) { counts.unused++; continue; }
                const c = m.cat || "other"; counts[c] = (counts[c] || 0) + 1;
            }
            tabs.innerHTML = [["all", "全部"]].concat(Object.keys(CAT_LABEL).map(c => [c, CAT_LABEL[c]]))
                ...
                .join("");
            tabs.querySelectorAll(".msel-tab").forEach(t => t.addEventListener("click", (e) => {
                e.stopPropagation();   // 重渲染会摘除本节点; 不阻断冒泡会被 window 外点关闭弹层
                mselCat = t.dataset.cat;
                mselScrollPos = 0;                  // 换分类回到顶部
                renderMselTabs(); renderMselList(); saveMselState();
            }));
        }
        ...
        mselFilter.addEventListener("input", () => { renderMselList(); mselScrollPos = 0; saveMselState(); });
        mselList.addEventListener("scroll", () => { mselScrollPos = mselList.scrollTop; saveMselState(); }, { passive: true });
        ...
        function saveState() {
            try {
                ...
                localStorage.setItem(STATE_KEY, JSON.stringify({
                    ...
                    // 地图选择器状态（分类/搜索词/列表滚动位置）一并记住
                    mselCat: mselCat, mselQ: mselFilter.value, mselScroll: mselScrollPos,
        ...
        async function init() {
            ...
            if (st) {
                ...
                // 地图选择器：分类 / 搜索词 / 滚动位置
                if (typeof st.mselCat === "string" && (st.mselCat === "all" || st.mselCat in CAT_LABEL))
                    mselCat = st.mselCat;
                if (typeof st.mselQ === "string") mselFilter.value = st.mselQ;
                if (typeof st.mselScroll === "number") mselScrollPos = st.mselScroll;
            }
...
def reachable_maps(workspace: str) -> set | None:
    """玩家从出生点出发、只靠正常游戏途径能到达的地图集合。

    途径 = 地图连接点 (MovementInfo 传送门) + NPC 对话传送 (NPCAction.Teleport,
    沿 NPCButton 页面图遍历 NPC 全部对话页)。出生点 = SafeZoneInfo.StartClass
    != None 的绑定区。不含 GM 命令/活动/道具等非世界结构途径。
    workspace 缺表时返回 None (调用方按全部可达处理)。"""
    registered = {str(r.get("FileName") or "").strip()
                  for r in _ws_rows(workspace, "MapInfo") if r.get("FileName")}
    if not registered:
        return None
    region2map = {}
    for r in _ws_rows(workspace, "MapRegion"):
        if r.get("Map"):
            region2map[r["Index"]] = str(r["Map"].get("Name", ""))
    # 地图连接点
    edges: dict = {}
    for m in _ws_rows(workspace, "MovementInfo"):
        s = region2map.get((m.get("SourceRegion") or {}).get("Index"))
        d = region2map.get((m.get("DestinationRegion") or {}).get("Index"))
        if s and d:
            edges.setdefault(s, set()).add(d)
    # NPC 对话页图 + 各页的 Teleport 目标
    btn: dict = {}
    for b in _ws_rows(workspace, "NPCButton"):
        p = (b.get("Page") or {}).get("Index")
        d = (b.get("DestinationPage") or {}).get("Index")
        if p is not None and d is not None:
            btn.setdefault(p, set()).add(d)
    tp_pages: dict = {}
    for a in _ws_rows(workspace, "NPCAction"):
        if a.get("ActionType") != "Teleport":
            continue
        p = (a.get("Page") or {}).get("Index")
        mp = (a.get("MapParameter1") or {}).get("Name")
        if p is not None and mp:
            tp_pages.setdefault(p, set()).add(str(mp))
    npc_edges: dict = {}
    for n in _ws_rows(workspace, "NPCInfo"):
        m = region2map.get((n.get("Region") or {}).get("Index"))
        if not m:
            continue
        seen, stack = set(), [(n.get("EntryPage") or {}).get("Index")]
        while stack:
            pg = stack.pop()
            if pg is None or pg in seen:
                continue
            seen.add(pg)
            npc_edges.setdefault(m, set()).update(tp_pages.get(pg, ()))
            stack.extend(btn.get(pg, ()))
    # 出生点
    starts = set()
    for z in _ws_rows(workspace, "SafeZoneInfo"):
        if z.get("StartClass") in (None, "None"):
            continue
        m = region2map.get((z.get("BindRegion") or {}).get("Index"))
        if m:
            starts.add(m)
    # BFS
    reach, queue = set(starts), list(starts)
    while queue:
        cur = queue.pop()
        for nxt in (edges.get(cur) or set()) | (npc_edges.get(cur) or set()):
            if nxt in registered and nxt not in reach:
                reach.add(nxt)
                queue.append(nxt)
    return reach
...
def api_maps_payload(maps_dir: str, layout: str) -> bytes:
    ...
    if body is None:
        # used = 玩家从出生点经正常途径(传送门+NPC对话传送)可达。
        # 不可达 = 未使用 (无论是否在服务器 MapInfo 登记)。
        reach = None
        for ws in (getattr(ViewerHandler, "db_workspace", None), DEFAULT_DBWORKSPACE):
            if ws:
                reach = reachable_maps(ws)
        ...
        for m in maps:
            ...
            m["used"] = (fid in reach) if reach is not None else True
...
def main():
    ...
    # /api/maps 的 used 判定用：服务器 MapInfo 收录集来源目录
    ViewerHandler.db_workspace = args.db_workspace
