...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  #bkmodal .btn { display:inline-block; margin-top:10px; padding:6px 14px; background:var(--panel2);
      border:1px solid var(--acc); color:var(--acc); border-radius:6px; text-decoration:none; cursor:pointer; }
  /* ---- UI 组装预览: 自绘控件（禁用系统原生控件） ---- */
  .zchk { display:inline-flex; align-items:center; gap:7px; cursor:pointer; font-size:13px; color:#fff;
          user-select:none; margin-right:20px; font-weight:normal; }
  .zchk .box { width:15px; height:15px; border:1.5px solid #6f7d8f; border-radius:4px;
  ...
  .zchk.on .box { border-color:#8ad4ff; background:#245b78; color:#fff; }
  .zchk:hover .box { border-color:#8ad4ff; }
  .zdd { position:relative; display:inline-block; }
  .zdd-btn { padding:5px 12px; background:var(--panel2); border:1px solid var(--line); color:var(--fg);
             border-radius:6px; cursor:pointer; font-size:12px; max-width:300px; white-space:nowrap;
             overflow:hidden; text-overflow:ellipsis; }
  .zdd-btn:hover { border-color:var(--acc); color:var(--acc); }
  .zdd-btn::after { content:"▾"; margin-left:7px; color:var(--dim); }
  .zdd-list { position:absolute; right:0; top:calc(100% + 4px); min-width:300px; max-width:420px;
              max-height:280px; overflow:auto; background:#141922; border:1px solid #3e5668;
              border-radius:8px; padding:4px; display:none; z-index:80;
              box-shadow:0 10px 30px rgba(0,0,0,.6); scrollbar-width:thin; }
  .zdd.open .zdd-list { display:block; }
  .zdd-item { display:flex; gap:10px; justify-content:space-between; padding:5px 8px; border-radius:5px;
              cursor:pointer; font-size:12px; color:var(--fg); white-space:nowrap; }
  .zdd-item:hover { background:var(--panel2); }
  .zdd-item.on { color:var(--acc); }
  .zdd-item .cnt { color:var(--dim); }
...
<div id="hud-modal" style="position:fixed; left:50%; top:50%; transform:translate(-50%,-50%); background:#181c24; border:2px solid #e8a33d; border-radius:12px; padding:20px; z-index:60; display:none; max-width:95vw; max-height:95vh; box-shadow:0 0 35px rgba(0,0,0,0.9);">
  ...
  <h3 style="color:#e8a33d; margin-bottom:12px; display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap;">
    ...
    <span id="chk-show-borders" class="zchk on" title="切换控件碰撞框显示"><span class="box">✓</span>显隐控件碰撞框</span>
  </h3>
  <div id="hud-ei">
  ...
  <div id="hud-zircon" style="display:none;">
    <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px; color:#aaa; font-size:12px;">
      ...
      <span style="margin-left:auto; display:flex; align-items:center; gap:6px;">窗口
        <span id="zwin-dd" class="zdd">
          <button id="zwin-btn" class="zdd-btn" type="button" aria-expanded="false">仅显示 Zircon HUD</button>
          <span id="zwin-list" class="zdd-list"></span>
        </span>
      </span>
    </div>
    <div id="zircon-stage-wrap" style="width:1024px; height:768px; overflow:hidden; background:#10151b; border:1px solid #3e5668; position:relative;">
    ...
    <div style="margin-top:8px; display:flex; justify-content:space-between; gap:12px; align-items:center;">
      ...
      <div style="color:#8ad4ff; font-size:12px; white-space:nowrap;"><span id="zircon-scale-label">100%</span> · Zircon UI Export</div>
    </div>
  </div>
</div>
...
// 自绘碰撞框开关（替代原生 checkbox；约定：预览内禁用系统原生控件）
let BORDERS_ON = true;
$('#chk-show-borders').onclick = function(){
  BORDERS_ON = !BORDERS_ON;
  this.classList.toggle('on', BORDERS_ON);
  toggleControlBorders(BORDERS_ON);
};
...
// 画布自适应: 1024×768 逻辑舞台按视口缩放，保证整个 HUD 都在弹窗内，不再溢出。
function fitZirconStage(){
  const modal = $('#hud-modal');
  if (!modal || modal.style.display === 'none' || HUD_VERSION !== 'zircon') return;
  const availW = Math.min(window.innerWidth * 0.92, 1024) - 46;   // 弹窗 padding+边框
  const availH = window.innerHeight * 0.90 - 150;                 // 标题/工具行/inspector/边距
  const scale = Math.max(0.15, Math.min(availW / 1024, availH / 768, 1));
  const wrap = $('#zircon-stage-wrap');
  wrap.style.width = Math.round(1024 * scale) + 'px';
  wrap.style.height = Math.round(768 * scale) + 'px';
  $('#zircon-stage').style.transform = `scale(${scale})`;
  $('#zircon-scale-label').textContent = Math.round(scale * 100) + '%';
}
window.addEventListener('resize', fitZirconStage);
...
// ---- 自绘窗口下拉（替代原生 select）----
function zwinToggle(open){
  const dd = $('#zwin-dd');
  dd.classList.toggle('open', open);
  $('#zwin-btn').setAttribute('aria-expanded', dd.classList.contains('open') ? 'true' : 'false');
}
$('#zwin-btn').onclick = e => {
  e.stopPropagation();
  zwinToggle(!$('#zwin-dd').classList.contains('open'));
};
document.addEventListener('click', e => {
  if (!e.target.closest('#zwin-dd')) zwinToggle(false);
});
function zwinSelect(cls, label){
  ZWIN_CURRENT = cls;
  $('#zwin-btn').textContent = label;
  document.querySelectorAll('.zdd-item').forEach(it => it.classList.toggle('on', it.dataset.cls === cls));
  zwinToggle(false);
  renderZirconWindow();
}
...
function zirconImage(library, index, x, y, title, parent){
  ...
  img.title = img.alt;
  img.style.cssText = `position:absolute;left:${x}px;top:${y}px;z-index:2;`;
...
// Zircon HUD: 所有坐标 = MainPanel.cs / GameScene.LayoutHud 的逻辑坐标 (1024×768 基准)。
function renderZirconHud(){
  ...
  mini.dataset.zcControl = '1';
  mini.title = 'MiniMapDialog · GameScene.LayoutHud: (vp.X - 200, 0)';
  mini.style.cssText = 'position:absolute;left:824px;top:0;width:200px;height:200px;border:1px dashed rgba(138,212,255,.55);background:rgba(10,20,30,.45);z-index:1;';
...
function renderZirconWindow(){
  ...
  layer.innerHTML = '';
  if (!ZWIN_CURRENT || !ZIRCON_UI) return;
  const w = ZIRCON_UI.windows.find(x => x.className === ZWIN_CURRENT);
  if (!w) return;
...
async function loadZirconPreview(){
  if (!ZIRCON_UI){
    try {
      ...
      const list = $('#zwin-list');
      list.innerHTML = '';
      const home = document.createElement('div');
      home.className = 'zdd-item on';
      home.dataset.cls = '';
      home.innerHTML = '<span>仅显示 Zircon HUD</span>';
      home.onclick = () => zwinSelect('', '仅显示 Zircon HUD');
      list.appendChild(home);
      for (const w of ZIRCON_UI.windows || []){
        const item = document.createElement('div');
        item.className = 'zdd-item';
        item.dataset.cls = w.className;
        const nm = document.createElement('span');
        nm.textContent = `${w.className} · ${w.title || ''}`;
        const cnt = document.createElement('span');
        cnt.className = 'cnt';
        cnt.textContent = `${w.controlCount} controls`;
        item.append(nm, cnt);
        item.onclick = () => zwinSelect(w.className, `${w.className} · ${w.title || ''}`);
...
<style>
  ...
  * { box-sizing:border-box; } body { margin:0; background:var(--bg); color:var(--fg); font:14px/1.45 "Microsoft YaHei",sans-serif; }
  ...
  .draw-call-legend { display:none; position:absolute; left:8px; top:210px; z-index:220; width:330px; max-height:260px; overflow:auto; padding:7px 9px; background:rgba(10,14,18,.92); border:1px solid #47d7ff; color:#e4e8ed; font:10px/1.4 monospace; pointer-events:none; }
  .show-draw-calls .draw-call-legend { display:block; }
  .hud-bars-legend { display:none; position:absolute; left:8px; top:34px; z-index:220; width:300px; padding:7px 9px; background:rgba(10,14,18,.90); border:1px solid #62e6a7; color:#e4e8ed; font:11px/1.45 monospace; pointer-events:none; }
  .show-layers .hud-bars-legend { display:block; }
...
<header>
  ...
  <label><input id="map-candidate-layer" type="checkbox"> 显示小地图资源候选层</label>
  <label><input id="layers" type="checkbox"> 显示绘制层级</label>
  <label><input id="drawcalls" type="checkbox"> 显示标准绘制调用</label>
...
function save(){localStorage.setItem(key,JSON.stringify({debug:$('#debug').checked,frames:$('#frames').checked,mode:SCREEN_MODE,diffShow:$('#diff-show').checked,diffOpacity:$('#diff-opacity').value,mapRect:$('#map-rect').checked,mapCandidate:$('#map-candidate-layer').checked,layers:$('#layers').checked,drawCalls:$('#drawcalls').checked,mapFile:MAP_FILE,mapFrame:MAP_FRAME}));}
...
function addFocusedResourceControl(x,y,w,h,frame,state,text,library){const lib=library||'GameInter.wil';const d=document.createElement('div');d.className='focus-resource-control';d.style.cssText=`left:${x}px;top:${y}px;width:${w}px;height:${h}px`;const im=document.createElement('img');im.src=`/api/image?f=${encodeURIComponent(lib)}&i=${frame}&scale=1`;im.alt=`${lib} Frame ${frame}`;im.style.cssText='width:100%;height:100%;image-rendering:pixelated';d.appendChild(im);if(state!=null){const tag=document.createElement('span');tag.textContent=`${text||''} · ${lib} F${frame}/${state}`;tag.style.cssText='position:absolute;left:0;top:100%;color:#bff7d0;background:rgba(8,12,16,.92);font:10px monospace;white-space:nowrap';d.appendChild(tag);}else if(text){const tag=document.createElement('span');tag.textContent=text;tag.style.cssText='position:absolute;left:0;top:100%;color:#bff7d0;background:rgba(8,12,16,.92);font:10px monospace;white-space:nowrap';d.appendChild(tag);}screen.appendChild(d);}
function addSkillCatalogPanel(l){const rows=l.magic_exp_records?.records||[];if(!rows.length)return;const panel=document.createElement('div');panel.style.cssText='position:absolute;left:500px;top:150px;width:286px;max-height:300px;overflow:auto;padding:7px 9px;color:#e4e8ed;background:rgba(8,12,16,.94);border:1px solid #e8a33d;font:10px/1.4 monospace;z-index:72;pointer-events:none';panel.innerHTML='<b style="color:#e8a33d">Magic.exp 解码技能目录（50条）</b>'+rows.slice(0,12).map(q=>{const lv=(q.levels||[]).slice(0,3).map(x=>`Lv${x.level}:${x.required_level}/${x.practice_value}`).join(' · ');return `<div style="margin-top:3px">#${esc(q.id)} ${esc(q.name)}<br><span style="color:#9ba7b4">${esc(q.attribute||'')} · ${esc(lv)}</span></div>`;}).join('')+'<div style="color:#ffcf70;margin-top:6px">目录来自原版 Magic.exp；当前类别/可见页仍需运行时字段绑定</div>';screen.appendChild(panel);}
function addSpecializedControlCatalogPanel(l,id){const rows=(l.specialized_control_rects||[]).filter(q=>q.window_id===id);if(!rows.length)return;const panel=document.createElement('div');panel.style.cssText='position:absolute;left:10px;bottom:10px;width:300px;max-height:150px;overflow:auto;padding:6px 8px;color:#d9f5e4;background:rgba(8,16,12,.94);border:1px solid #62e6a7;font:10px monospace;z-index:73;pointer-events:none';panel.innerHTML=`<b style="color:#62e6a7">统一专项控件目录 · ${esc(id)}</b>`+rows.map((q,i)=>`<div>${String(i+1).padStart(2,'0')} · F${esc((q.frame_pair||[]).join('/'))} · ${esc(q.resource_library)} · (${q.relative_rect.join(',')}) · [${esc(q.evidence_level)}]</div>`).join('');screen.appendChild(panel);}
function addFocusedWindow(r,analysis){
  ...
  if(r.id==='window.inventory'){
    addFocusedResourceControl(ox+249,oy+288,28,26,161,162,'背包关闭');
    addFocusedResourceControl(ox+176,oy+262,64,20,264,265,'背包操作');
    addFocusedResourceControl(ox+176,oy+286,76,88,267,268,'人物外观资源候选','Interface1c.wil');
    ...
    addFocusedResourceControl(ox+212,oy+298,28,26,161,162,'状态窗口关闭');
    addFocusedResourceControl(ox+176,oy+286,36,36,171,172,'状态操作');
    ...
    for(const q of pc){if(q.object_offset==='this+0x6C')continue;const pair=(ce.controls||[]).find(x=>x.object_offset===q.object_offset)?.frame_pair||[];const w=pair.length&&[360,362,364,366,368,370].includes(pair[0])?36:40;const h=pair.length&&[360,362,364,366,368,370].includes(pair[0])?34:20;if(pair.length)addFocusedResourceControl(ox+q.relative_position[0],oy+q.relative_position[1],w,h,pair[0],pair[1],q.role);else addFocusedGeometry(ox+q.relative_position[0],oy+q.relative_position[1],w,h,`${q.role} · ${q.object_offset}`);}
  ...
  }else if(r.id==='window.skill-book'){
  ...
  }else if(r.id==='window.exchange-candidate'){
    const se=evidenceWindow(r.id),split=se.parent_split_geometry;
    ...
    const se=evidenceWindow(r.id),pc=se.paint_repositioned_controls?.records||[];
    for(const q of pc){const pair=q.frames||[];const w=pair[0]===860?44:(pair[0]===862||pair[0]===864?60:pair[0]===866?56:28);const h=pair[0]>=860?20:26;if(pair.length)addFocusedResourceControl(ox+q.position[0],oy+q.position[1],w,h,pair[0],pair[1],q.role);else addFocusedGeometry(ox+q.position[0],oy+q.position[1],w,h,q.role);}
  }
  addSpecializedControlCatalogPanel(DATA.layout,r.id);
  ...
  const local=(r.id==='window.horse'?DATA.layout.horse_window_evidence?.paint_order:null)||(r.id==='window.status'?evidenceWindow(r.id)?.paint_state?.paint_order:null)||(r.id==='window.inventory'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.skill-book'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.quest'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.store-candidate'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.chat-pop'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.npc-candidate'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.option'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.group'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.guild-candidate'?evidenceWindow(r.id)?.paint_order:null)||(r.id==='window.exchange-candidate'?evidenceWindow(r.id)?.paint_order:null);
  if(local?.length){const panel=document.createElement('div');panel.style.cssText='position:absolute;right:10px;top:40px;width:260px;max-height:220px;overflow:auto;padding:6px 8px;color:#c9f5d0;background:rgba(8,16,12,.92);border:1px solid #62e6a7;font:10px monospace;z-index:72;pointer-events:none';panel.innerHTML='<b style="color:#62e6a7">原版局部绘制顺序</b>'+local.map(q=>`<div>${esc(String(q.order).padStart(2,'0'))} · ${esc(q.role||q.target||'draw')}</div>`).join('');screen.appendChild(panel);}
}
...
function addSecondaryScreen(l,index){const candidate=(l.secondary_screen_candidates||[])[index];if(!candidate)return;const bg=candidate.interface1c_frames?.['50']||{width:640,height:480};const ox=(800-bg.width)/2,oy=(600-bg.height)/2;const image=document.createElement('img');image.src='/api/image?f=Interface1c.wil&i=50&scale=1';image.style.cssText=`position:absolute;left:${ox}px;top:${oy}px;width:${bg.width}px;height:${bg.height}px`;image.title=`Interface1c.wil Frame 50 · secondary screen candidate ${index} · anchor (${bg.offsetX??'—'},${bg.offsetY??'—'})`;screen.appendChild(image);const scope=index===0?'interface1c-cluster-0x456d':'interface1c-cluster-0x4027';for(const r of (l.secondary_control_constructors||[])){if(r.scope!==scope)continue;const copy={...r,position:{...r.position},size:{...r.size}};copy.position.x+=ox;copy.position.y+=oy;copy.anchor={offsetX:r.resource?.offsetX??null,offsetY:r.resource?.offsetY??null};addSecondaryControl(copy);}const anchor=document.createElement('div');anchor.style.cssText=`position:absolute;left:${ox}px;top:${oy}px;width:${bg.width}px;height:${bg.height}px;border:1px dashed rgba(183,140,255,.75);z-index:36;pointer-events:none`;const anchorLabel=document.createElement('span');anchorLabel.textContent=`Frame 50 resource bounds · WIL anchor (${bg.offsetX??'—'},${bg.offsetY??'—'}) · transparent pixels remain transparent`;anchorLabel.style.cssText='position:absolute;left:3px;bottom:3px;color:#e2d4ff;background:rgba(15,10,25,.86);font:10px monospace;padding:2px 4px';anchor.appendChild(anchorLabel);screen.appendChild(anchor);const label=document.createElement('div');label.style.cssText='position:absolute;left:16px;top:14px;color:#e8a33d;font:12px monospace;z-index:70';label.textContent=`[Interface1c secondary candidate ${index} · ${bg.width}×${bg.height} centered in 800×600 · WIL anchor (${bg.offsetX??'—'},${bg.offsetY??'—'}) · position is preview placement, not upgraded screen evidence]`;screen.appendChild(label);}
function mapNames(l,lib,frame){const rows=l.map_ui_evidence?.server_cross_reference_rows||[];const names=rows.filter(q=>q.library===lib&&Number(q.frame)===Number(frame)).flatMap(q=>q.server_map_names||[]);return [...new Set(names)].join(' / ');}
function addMapCandidate(l){const image=document.createElement('img');image.src='/api/image?f=FMMap.wil&i=0&scale=1';image.style.cssText='position:absolute;left:0;top:34px;width:800px;height:533px;image-rendering:auto';image.title='FMMap.wil Frame 0 · full-map resource candidate';screen.appendChild(image);const label=document.createElement('div');label.style.cssText='position:absolute;left:14px;top:10px;color:#e8a33d;background:rgba(10,14,18,.88);padding:4px 6px;border:1px solid #e8a33d;font:12px monospace;z-index:70';const name=mapNames(l,'FMMap.wil',0);label.textContent=`[FMMap.wil Frame 0${name?' · '+name:''} · 1200×800 source scaled to 800×533 · resource candidate only]`;screen.appendChild(label);}
function addMapSelectionCandidate(l){const image=document.createElement('img');image.src=`/api/image?f=${encodeURIComponent(MAP_FILE)}&i=${MAP_FRAME}&scale=1`;image.style.cssText='position:absolute;left:0;top:34px;width:800px;height:533px;image-rendering:auto';image.title=`${MAP_FILE} Frame ${MAP_FRAME} · selected resource candidate`;screen.appendChild(image);const label=document.createElement('div');label.style.cssText='position:absolute;left:14px;top:10px;color:#e8a33d;background:rgba(10,14,18,.88);padding:4px 6px;border:1px solid #e8a33d;font:12px monospace;z-index:70';const name=mapNames(l,MAP_FILE,MAP_FRAME);label.textContent=`[${MAP_FILE} Frame ${MAP_FRAME}${name?' · '+name:''} · selected map resource · scaled candidate only]`;screen.appendChild(label);}
...
function addStoreControlEvidence(e,stateNo){const rows=e.controls||[];if(!rows.length)return;const panel=document.createElement('div');panel.style.cssText='position:absolute;right:10px;top:40px;width:300px;max-height:250px;overflow:auto;padding:6px 8px;color:#e4e8ed;background:rgba(8,12,16,.94);border:1px solid #e8a33d;font:10px monospace;z-index:72;pointer-events:none';panel.innerHTML=`<b style="color:#e8a33d">商店/仓库状态${stateNo} 控件构造证据</b>`+rows.map((q,i)=>{const f=q.frame_pair||[];const p=q.raw_position||{};return `<div style="margin-top:4px"><img src="/api/image?f=GameInter.wil&i=${f[0]}&scale=1" style="width:24px;height:18px;image-rendering:pixelated;vertical-align:middle"> #${i+1} F${f.join('/')} · ${esc(p.x||'')} / ${esc(p.y||'')}</div>`;}).join('')+'<div style="color:#ffcf70;margin-top:5px">raw arg4/arg5 相对表达式；工厂会重新计算父窗口 Rect</div>';screen.appendChild(panel);}
function addMapMinimapCandidate(l){const r=l.map_ui_evidence?.viewport?.fixed_minimap_widget?.screen_rect||{left:672,top:0,right:800,bottom:128};const frame=document.createElement('div');frame.style.cssText=`position:absolute;left:${r.left}px;top:${r.top}px;width:${r.right-r.left}px;height:${r.bottom-r.top}px;overflow:hidden;background:#08110e;border:2px solid #e8a33d;z-index:24`;const image=document.createElement('img');image.src=`/api/image?f=${encodeURIComponent(MAP_FILE)}&i=${MAP_FRAME}&scale=1`;image.style.cssText='width:128px;height:128px;object-fit:contain;image-rendering:auto;opacity:.92';frame.appendChild(image);const label=document.createElement('div');label.style.cssText='position:absolute;left:4px;top:4px;color:#ffd58a;background:rgba(10,14,18,.88);padding:2px 3px;font:10px monospace;z-index:2';const name=mapNames(l,MAP_FILE,MAP_FRAME);label.textContent=`${MAP_FILE} F${MAP_FRAME}${name?' · '+name:''} → 128×128`;frame.appendChild(label);screen.appendChild(frame);const note=document.createElement('div');note.style.cssText='position:absolute;left:14px;top:10px;color:#e8a33d;background:rgba(10,14,18,.88);padding:4px 6px;border:1px solid #e8a33d;font:12px monospace;z-index:70';note.textContent=`[固定小地图目标 Rect (${r.left},${r.top})-(${r.right},${r.bottom}) · ${MAP_FILE} F${MAP_FRAME}${name?' · '+name:''} 仅作资源缩放候选 · 边框/裁剪/标记仍未冒充已确认]`;screen.appendChild(note);}
function addPromptEvidence(mode){if(mode==='prompt.confirmation'){const e=DATA.layout.confirmation_prompt_evidence||{};const p=e.parent_window||{};const [x,y]=p.screen_origin||[220,151];const [w,h]=p.frame_size||[360,190];const box=document.createElement('div');box.style.cssText=`position:absolute;left:${x}px;top:${y}px;width:${w}px;height:${h}px;overflow:hidden;border:1px solid #e8a33d;z-index:24`;const im=document.createElement('img');im.src=`/api/image?f=GameInter.wil&i=${p.background?.frame||950}&scale=1`;im.style.cssText=`position:absolute;left:0;top:0;width:${w}px;height:${h}px`;box.appendChild(im);screen.appendChild(box);const frames={151:151,157:157,154:154};for(const q of (e.child_hit_rects?.records||[])){const [rx,ry,rw,rh]=q.relative_rect||[];const frame=frames[q.frame_pair?.[0]]||q.frame_pair?.[0];if([rx,ry,rw,rh,frame].some(v=>v==null))continue;const b=document.createElement('div');b.style.cssText=`position:absolute;left:${x+rx}px;top:${y+ry}px;width:${rw}px;height:${rh}px;border:1px dashed #62e6a7;background:rgba(40,190,120,.08);z-index:48`;const bi=document.createElement('img');bi.src=`/api/image?f=GameInter.wil&i=${frame}&scale=1`;bi.style.cssText='width:100%;height:100%;image-rendering:pixelated';b.appendChild(bi);screen.appendChild(b);}const label=document.createElement('div');label.style.cssText='position:absolute;left:14px;top:10px;color:#e8a33d;background:rgba(10,14,18,.88);padding:4px 6px;border:1px solid #e8a33d;font:12px monospace;z-index:70';label.textContent=`[F${p.background?.frame||950} · ${w}×${h} · -1/-1 center rule → (${x},${y}) · child rects from evidence JSON]`;screen.appendChild(label);}else{const box=document.createElement('div');box.style.cssText='position:absolute;left:107px;top:110px;width:584px;height:252px;overflow:hidden;border:1px solid #e8a33d;z-index:24';const im=document.createElement('img');im.src='/api/image?f=GameInter.wil&i=602&scale=1';im.style.cssText='position:absolute;left:0;top:0;width:1024px;height:256px';box.appendChild(im);screen.appendChild(box);[['[行会修改 请自行修改行会等级、成员排行信息]',23,94,'#ffd58a'],['[行会公告，请自行修改公告内容.]',23,94,'#9ed0ff']].forEach(([txt,rx,ry,color],i)=>{const t=document.createElement('div');t.style.cssText=`position:absolute;left:${107+rx}px;top:${110+ry+i*20}px;color:${color};background:rgba(8,12,16,.72);font:12px sans-serif;z-index:48;white-space:nowrap`;t.textContent=`state${i} · ${txt}`;screen.appendChild(t);});const label=document.createElement('div');label.style.cssText='position:absolute;left:14px;top:10px;color:#e8a33d;background:rgba(10,14,18,.88);padding:4px 6px;border:1px solid #e8a33d;font:12px monospace;z-index:70';label.textContent='[F602 · parent (107,110) · 584×252 · 行会公告/行会修改候选文字 · state分支]';screen.appendChild(label);}}
...
function addLayerLegend(l){const box=document.createElement('div');box.className='layer-legend';const layers=l.draw_order_evidence?.layers||[];box.innerHTML='<b>原版绘制层级候选</b><br>'+layers.map(x=>`<div>${esc(String(x.order).padStart(3,'0'))} · ${esc(x.id)} <span style="color:#9ba7b4">[${esc(x.confidence||'candidate')}]</span></div>`).join('');screen.appendChild(box);addVisibleWindowDispatch(l);addHudBarDispatch(l);if($('#drawcalls')?.checked)addNormalizedDrawCallLegend(l);}
function addNormalizedDrawCallLegend(l){const rows=l.normalized_draw_calls||[];if(!rows.length)return;const box=document.createElement('div');box.className='draw-call-legend';box.innerHTML='<b style="color:#47d7ff">normalized_draw_calls · scope/order/VA</b>'+rows.slice(0,24).map((x,i)=>`<div>${String(i+1).padStart(2,'0')} · ${esc(x.scope||'scope')} · ${esc(String(x.order??'—'))} · ${esc(x.va||'—')} · ${esc(x.role||'draw')} <span style="color:#9ba7b4">[${esc(x.evidence_level||'candidate')}]</span></div>`).join('')+(rows.length>24?`<div style="color:#ffcf70;margin-top:4px">其余 ${rows.length-24} 条保留在 layout.json</div>`:'');screen.appendChild(box);}
function addVisibleWindowDispatch(l){const rows=l.window_traversal?.render_dispatch?.dispatch_table||[];if(!rows.length)return;const box=document.createElement('div');box.style.cssText='position:absolute;left:10px;bottom:10px;width:310px;max-height:180px;overflow:auto;padding:6px 8px;color:#d9e5ef;background:rgba(8,12,16,.94);border:1px solid #62e6a7;font:10px monospace;z-index:72;pointer-events:none';box.innerHTML='<b style="color:#62e6a7">可见窗口链表 → 绘制分派（运行时）</b>'+rows.map((q,i)=>`<div>${String(i+1).padStart(2,'0')} · ID${q.window_id} · ${esc(q.role||'window')} · ${esc(q.paint_va)}</div>`).join('')+'<div style="color:#ffcf70;margin-top:4px">顺序随显示/隐藏/置顶状态变化，不是固定启动层级</div>';screen.appendChild(box);}
function addHudBarDispatch(l){const rows=l.hud_bars_render_evidence?.draw_sequence||[];if(!rows.length)return;const box=document.createElement('div');box.className='hud-bars-legend';box.innerHTML='<b style="color:#62e6a7">HUD 动态资源绘制顺序（原版静态）</b>'+rows.map(q=>`<div>${String(q.order).padStart(2,'0')} · ${esc(String(q.resource_frame||'text'))} · ${esc(q.role||'draw')} · ${esc(q.va||'')}</div>`).join('')+'<div style="color:#ffcf70;margin-top:4px">动态裁剪方向/运行时数值仍保持 pending</div>';screen.appendChild(box);}
...
function load(){const s=state();if(typeof s.debug==='boolean')$('#debug').checked=s.debug;if(typeof s.frames==='boolean')$('#frames').checked=s.frames;if(typeof s.mode==='string')SCREEN_MODE=s.mode;if(typeof s.diffShow==='boolean')$('#diff-show').checked=s.diffShow;if(s.diffOpacity!=null)$('#diff-opacity').value=s.diffOpacity;if(typeof s.mapRect==='boolean')$('#map-rect').checked=s.mapRect;if(typeof s.mapCandidate==='boolean')$('#map-candidate-layer').checked=s.mapCandidate;if(typeof s.layers==='boolean')$('#layers').checked=s.layers;if(typeof s.drawCalls==='boolean')$('#drawcalls').checked=s.drawCalls;$('#mode').value=SCREEN_MODE;screen.classList.toggle('show-layers',$('#layers').checked);screen.classList.toggle('show-draw-calls',$('#drawcalls').checked);$('#mode').onchange=()=>{SCREEN_MODE=$('#mode').value;save();render()};$('#layers').onchange=()=>{screen.classList.toggle('show-layers',$('#layers').checked);save()};$('#drawcalls').onchange=()=>{screen.classList.toggle('show-draw-calls',$('#drawcalls').checked);save();render()};$('#map-rect').onchange=()=>{save();render()};$('#map-candidate-layer').onchange=()=>{save();render()};$('#diff-show').onchange=()=>{save();render()};$('#diff-opacity').oninput=()=>{save();render()};$('#diff-file').onchange=()=>{const f=$('#diff-file').files?.[0];if(!f)return;const reader=new FileReader();reader.onload=()=>{DIFF_URL=String(reader.result||'');$('#diff-show').checked=true;save();render()};reader.readAsDataURL(f)};$('#debug').onchange=()=>{screen.classList.toggle('debug',$('#debug').checked);save()};$('#frames').onchange=()=>{screen.classList.toggle('no-frames',!$('#frames').checked);save()};$('#reset').onclick=()=>{localStorage.removeItem(key);location.reload()};fetch('/api/ui-layout').then(r=>r.json()).then(d=>{DATA=d;render()}).catch(e=>$('#summary').textContent='读取布局失败：'+e);}
const basePromptEvidence = addPromptEvidence;
function addPromptEvidenceWithPanel(mode){
  ...
  [[161,162,655,126,28,26,'F602 子控件 / 关闭或确认候选'],[606,607,603,153,40,20,'F606 子控件 / 状态候选']].forEach(([frame,state,x,y,w,h,text])=>{
    ...
    b.style.cssText=`position:absolute;left:${x}px;top:${y}px;width:${w}px;height:${h}px;border:1px dashed #62e6a7;background:rgba(40,190,120,.08);z-index:49`;
    const im=document.createElement('img');im.src=`/api/image?f=GameInter.wil&i=${frame}&scale=1`;im.style.cssText='width:100%;height:100%;image-rendering:pixelated';b.appendChild(im);
    const label=document.createElement('span');label.textContent=`${text} · ${frame}/${state} · (${x},${y},${w},${h})`;label.style.cssText='position:absolute;left:0;top:100%;color:#9ff0c2;background:rgba(8,12,16,.86);font:10px monospace;white-space:nowrap';b.appendChild(label);screen.appendChild(b);
  });
};
let renderEvidenceLayout = render;
...
renderEvidenceLayout=function(){baseRenderEvidenceLayout();addHudLabelEvidence(DATA?.layout||{});addWindowRegistryEvidence(DATA?.layout||{});if($('#map-candidate-layer')?.checked)addMapMinimapCandidate(DATA.layout);};
render = function(){
  ...
  if(SCREEN_MODE.startsWith('window.store-state-')){
    ...
    addStoreControlEvidence(DATA.layout.store_window_evidence||{},Number(wanted.slice('window.store-state-'.length)));
    return;
  }
  if(SCREEN_MODE==='prompt.confirmation'||SCREEN_MODE==='prompt.notice'){
    screen.querySelectorAll(':scope > *:not(#world-label)').forEach(x=>x.remove());
    addPromptEvidenceWithPanel(SCREEN_MODE);
    return;
  }
