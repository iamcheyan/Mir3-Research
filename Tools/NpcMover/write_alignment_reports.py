...
def main() -> int:
    ...
    report = [
        ...
        f"- Hero-kill 地图源搜索审计：本地 EI 运行时源图 {source_search.get('searched_roots', {}).get('/home/tetsuya/mir2ei/Map', {}).get('map_file_count', '未知')} 个；匹配刷新中无本地源图 {source_search.get('manifest_counts', {}).get('matched_respawns_without_local_source', '未知')} 条；额外 Zircon Debug 副本 {source_search.get('searched_roots', {}).get('/home/tetsuya/development/Debug/ServerCore/Map', {}).get('map_file_count', '未知')} 个，缺失 Hero-kill 名称的独立变体 {source_search.get('additional_zircon_copy_check', {}).get('missing_hero_map_name_variant_count', '未知')} 个；挂载/临时目录命中 {source_search.get('additional_searches', {}).get('mounts_and_temp', {}).get('exact_missing_map_hits', '未知')} 个，客户端打包资源命中 {source_search.get('additional_searches', {}).get('packed_resource_name_scan', {}).get('hits', '未知')} 个；未发现新的研究缓存、NAS 或独立归档源图。",
        "",
        "## 8. 未决项与人工复核",
        "",
