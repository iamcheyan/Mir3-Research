...
| 背包根框/网格 | `inventory-window-render-evidence.json`：F250，根 284×324，六列、六行可视区；记录表与可视占位格分离；`bag-list-fill-chain-evidence.json`：46 条记录、首格标记和跨格占位 | `InventoryDialog` 使用 F250、284×324、六列六行可视；`DXItemGrid.UseLegacyFootprints` 依据 `Inventory.wil` 帧尺寸 first-fit 生成占位锚点，`DXItemCell` 将记录槽位与可视格索引分离 | 2026-09-25 真实运行截图 `inventory-multicell-runtime.png` 显示 Armour 图标跨 2 列×3 行且占位格未重复绘制；服务端仍只提供记录槽位，未提供 EI 原始 cell-table 字段 | 多格 footprint 运行表现已闭合；服务器原始 cell-table 语义仍保留为证据限制；高 |
