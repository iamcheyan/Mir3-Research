...
| 经验条 | `hud-bars-render-evidence.json`：F63；`primary-main-hud-setrect.md`：屏幕 `(61,586)-(400,597)`，相对 HUD `(61,121)`，填充按经验比例 | `MainPanel.ExperienceBar` 使用 F63、339×11、`(61,121)`，`DrawExperienceFill` 按当前/最大经验裁切绘制 | 旧根自动尺寸为 1024×68 时，F63 位于根外，形成“经验条错位/不可见”；当前已固定 800×136 | 代码已修；最新 `LegacyHud PASS` 报告 viewport `1024×768` 和 F63 几何；动态填充长度尚未用第二经验值复核 |
...
| 右侧聊天入口 | EI cap9/id8 进入 F350；F350 根 572×388、19 行历史、输入框和 6 个命令控件 | `MainPanel.MailButton` 在 legacy 分支调用 `_legacyChatDialog.OpenChat/CloseChat`；R 键在焦点保护前切换同一窗口，关闭时释放 Viewport focus | 原实现只切换 `_chatLog.Visible`；关闭后 LineEdit focus owner 会吞掉 R | 鼠标打开、关闭、再次鼠标打开已通过：`chat-click-runtime.png`、`chat-close-button-runtime.png`、`chat-close-click-reopen-correct.png`；R 重开在当前 Xvfb 注入仍未产生可见窗口，保留待验 |
...
2. 已取得 F350 直达、HUD 鼠标打开、关闭按钮关闭、再次鼠标打开的真实截图；普通公开聊天文本不回显发送者自身，单客户端无法完成 `ReceiveChat → ChatLogPanel` 的普通文本视觉闭环；R 重开仍受当前 Xvfb 键盘注入影响。
3. 背包真实物品布局已截图，F280 轨道可见；本轮拖柄/滚轮注入导致窗口消失，未宣称滚动和边界通过；服务端记录索引到 EI 原始列/行仍未完全重建。
4. 人物收起/展开真实截图已通过；Weapon/Armour/Necklace 大 hit record 行为仍未迁移。
