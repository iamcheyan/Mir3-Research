...
| 底部聊天栏 | `RESEARCH_LOG.md:2819`：F50 内主 HUD 聊天总区为屏幕 `(224,492)-(578,566)`，相对 F50 根 `(224,27)`、`354×74`；`chat-window-unified-model.json` / `chat-window-render-evidence.json` 只描述独立 F350 详细窗 | `GameScene.ReceiveChat → AddChatMessage → _chatLog.AddMessage` 与 `_legacyChatDialog.AddMessage` 共享同一接收事件；legacy `_chatLog` 现固定 `354×74`、裁剪并锚到 `_mainPanel.Position+(224,27)`；legacy `OnChat` 直接使用服务端已包装的 `p.Text`，不再重复添加发送者/类型前缀 | 原根因是 `_chatLog` 的现代 `400×150` 几何脱离 F50 聊天槽；消息链本身存在。修复后真实日志显示 `hudVisible=True`、连续 `hudMessages/hudLines` 增长，普通消息和 89 字长消息均进入 HUD；`ChatLogPanel` 的 `14px` 最小行高与 `ClipContents` 保持槽内裁剪 | 已修；1024×768 真实登录截图：`chat-hud-fixed-baseline.png`、`chat-hud-r-message-final.png`、`chat-hud-long-message-fixed.png`；普通消息发送者只出现一次 |
...
5. **当前根因判定**：`GameScene.OnChat` 已订阅 `ChatEvent`，`ReceiveChat/AddChatMessage` 已同时写 `_chatLog` 和 `_legacyChatDialog`；既有运行日志也看到 `Net 入队: Chat`。可见差异是 `_chatLog` 仍为现代 `400×150`、被锚到 F50 上方，而 F50 槽位在 `(224,27)`；此外 legacy `OnChat` 对服务端已包装的 `p.Text` 重复添加发送者。故本轮先修共享 legacy 几何和 legacy 文本包装，不改服务端可见范围或用透明度掩盖。
...
- 以完整 `1024×768` viewport、`--legacy-ui --legacy-hud` 真实登录；先建立同图同 `MaxViewRange` 的普通聊天发送端，再验证客户端 `StartGame Result=Success`、`ChatEvent`、`[LegacyChatHud]` 几何/消息日志。
