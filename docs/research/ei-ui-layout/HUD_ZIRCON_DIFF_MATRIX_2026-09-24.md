...
| 右侧聊天入口 | EI cap9/id8 进入 F350；F350 根 572×388、19 行历史、输入框和 6 个命令控件 | `MainPanel.MailButton` 在 legacy 分支调用 `_legacyChatDialog.OpenChat/CloseChat`；R 键在焦点保护前切换同一窗口，关闭时释放 Viewport focus | R 事件已在日志确认两次到达：第一次关闭成功，第二次进入 `OpenChat` 且 `Visible/IsVisibleInTree=true`，但重开截图无 F350 绘制；不是键盘事件未到达，而是窗口重复显示后的渲染/生命周期问题 | 鼠标入口、关闭按钮、直达打开已通过；`chat-r-diagnostic-closed.png` 证明 R 关闭，`chat-r-fixed-reopened.png` 证明 R 重开未绘制；保留为高优先级待修 |
...
2. 已取得 F350 直达、HUD 鼠标打开、关闭按钮关闭、再次鼠标打开的真实截图；普通公开聊天文本不回显发送者自身，单客户端无法完成 `ReceiveChat → ChatLogPanel` 的普通文本视觉闭环。
3. R 键事件已确认到达并关闭 F350；第二次 R 也进入 `OpenChat`，但 F350 重开后不绘制，需修复窗口重复显示生命周期。
