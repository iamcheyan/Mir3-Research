...
| 人物属性面板 | `status-window-render-evidence.json`：第一列 13 个标签/格式项，起点 `(x+0xFF,y+0x43)`、行距 15；第二列 11 项，起点 `(x+0x17F,y+0x1E)`、行距 15 | 扩展态创建两列共 24 个可见文本项，使用 `PlayerStats`、当前 HP/MP、经验和负重；无法映射的中毒恢复/生命恢复/魔法恢复显示 `—`，不猜值 | 旧实现为 7 项/12 项、22px 单列，且和 F201 艺术层重叠；原版魔法与魔法防御力的多值语义仍未由当前服务端字段独立闭合 | 几何/字段覆盖已改；无证据字段已保守处理；剩余多值语义为证据阻塞，高 |
...
6. 人物属性原始全局字段到 Zircon `Stat` 的已证字段映射已保留；“中毒恢复/生命恢复/魔法恢复”无法独立闭合，现统一显示 `—`，见 Round 789 与 Zircon commit `e3b75105`。原版魔法/魔法防御力多值字段仍维持证据阻塞，不猜测服务端语义。
