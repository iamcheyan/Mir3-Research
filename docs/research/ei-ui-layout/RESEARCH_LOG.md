...
重新按 `0x0042EB7F` 的调用顺序整理背包动态内容：窗口先以 Frame 94 在 `(window.x+0xF8, window.y-0xA5)` 进入顶部/详情组合，然后检查 `0x007DA1C0/0x007DA1C4` 与 `0x007243D8`，用 `0x0042F150` 命中 36px 网格、再由 `0x0042F2A0` 把线性索引拆成 `column=index%6`、`row=floor(index/6)`，最终把源/目标矩形送入 `0x00466800` 和 `0x004542F0`。
...
因此“背包是 6×6 物品内容窗口、不是纯按钮网格”的原版内容证据已经可直接提供给 UI 还原器；仍不把主数值或打包字段命名为现代数据库字段，也不把 Frame 94 的业务用途从坐标表达式扩大解释。以上映射已同步到 `inventory-window-render-evidence.json`。
