...
## Round 601 (F907) — 2026-08-12：HANDOFF 刷新 113（Round 599-600）
...
## Round 613 (F919) — 2026-08-12：HANDOFF 刷新 117（Round 610-612）
...
## Round 616 (F922) — 2026-08-12：HANDOFF 刷新 118（Round 613-615）
...
## Round 641 (F947) — 2026-08-12：HANDOFF 刷新 127（Round 638-640）

- **〔刷新〕**HANDOFF 追加 Round 638-640（F945-F946：行会窗口输入/重置 + 行会窗口闭合）；基线 Round 637=ad68532 → Round 640=29856f5（**605 连发 F335-F946**）。
- 落盘：handoff-refresh-127-evidence.json（F947，derived-tooling）+ RESEARCH_LOG + UI_COMPLETION_AUDIT Round 641。
## Round 642 (F948) — 2026-08-12：组队窗口输入 + 消息分派（F845/F897 依赖）
...
## Round 644 (F950) — 2026-08-12：HANDOFF 刷新 128（Round 641-643）
...
## Round 647 (F953) — 2026-08-12：HANDOFF 刷新 129（Round 644-646）
...
## Round 655 (F961) — 2026-08-12：对话 + 公告窗口闭合（F746/F857/F957 + F854/F866/F870/F958 汇总）

- **〔验证〕**compileall OK、verify exit 0、node OK、JSON 有效、**585 证据 JSON**；三服务 200；master 3537591（**619 连发 F335-F960 已推**）。
- **〔弧〕**对话（F746 选项 + F857 绘制 + F957 输入）+ 公告（F854 渲染 + F866 节点 + F870 按键 + F958 输入）——**双窗口完整**；**13/13 窗口输入表面 100%**。
- 落盘：dialog-announcement-closure-evidence.json（F961，derived）+ RESEARCH_LOG + UI_COMPLETION_AUDIT Round 655。
## Round 656 (F962) — 2026-08-12：HANDOFF 刷新 132（Round 654-655）
...
## Round 676 (F982) — 2026-08-12：技能书闭合（F839/F848/F851/F981 汇总）

- **〔验证〕**compileall OK、verify exit 0、node OK、JSON 有效、**606 证据 JSON**；三服务 200；master 1ad1eba（**640 连发 F335-F981 已推**）。
- **〔弧〕**绘制（F839）+ 页签头（F848）+ 文本解析器（F851）+ 绘制/页（F981）——**技能书完整**（3 页签 + 8 槽 + 页 ÷3 + 文本解析器 + GDI）；技能表面 100%。
- 落盘：skill-book-closure-evidence.json（F982，derived）+ RESEARCH_LOG + UI_COMPLETION_AUDIT Round 676。
## Round 677 (F983) — 2026-08-12：HANDOFF 刷新 139（Round 674-676）
...
## Round 686 (F992) — 2026-08-12：HANDOFF 刷新 142（Round 683-685）

- **〔刷新〕**HANDOFF 追加 Round 683-685（F990-F991：HP 条 blit 全量 + HP 条系统闭合）；基线 Round 682=ec09a52 → Round 685=b8d775e（**650 连发 F335-F991**）。
- 落盘：handoff-refresh-142-evidence.json（F992，derived-tooling）+ RESEARCH_LOG + UI_COMPLETION_AUDIT Round 686。
## Round 687 (F993) — 2026-08-12：实体投影 + 阴影 blit 全量（F894）
...
## Round 700 (F1006) — 2026-08-12：**ROUND 700 里程碑**
...
## Round 718 (F1024) — 2026-08-12：recv1 闭合（F836/F797/F1023 汇总）
...
## Round 726 (F1032) — 2026-08-12：实体/列表析构 vtable 尾部全量（F440 扩展）
...
## Round 735 (F1041) — 2026-08-12：共享 widget vtable 家族全量（F752/F917/F987 扩展）
...
## Round 751 (F1057) — 2026-08-12：实体子类 0x476480 核心方法四连（F1054 后续）

- **〔0x40C620 战斗状态块重置〕**call 0x404A30 后清零 [+0x629C8..+0x62A3C] ~0x78 字节（旗标 +0x629CD-D3、双 0x10 零块、[+0x62A38]=−1）——每实体战斗暂存区。
- **〔0x40CA20 动作状态机〕**门 [0xB4]+1==[0xC4] && [0xC0]==0xF → 朝向查表 0x40CC5C + jt 0x40CC50 → 槽 0x46-0x49（[0x8A]==3 时 +0xA）；行走态 [0xC0]∈9..0xE/0x10/0x11 → 待定槽 0x89/0x8C/0x8D；移动回显 [0x629D0]==2 → **发送 0x82（主体）/0x83（英雄）坐标 [0xCC]/[0xD0]** 经 0x45AFC0 @ 0x8AB130。
...
## Round 765 (F1071) — 2026-08-12：双图对象架构 + 方法族 + 0x574118 死对象定案（F1070 线索闭合）

- **〔方法族全解〕**0x43CFD0 世界→屏幕（(w−a)×48−scroll−0xC8 / ×32−0x9D）、0x43D020 逆变换（/48 /32 魔数 0x2AAAAAAB）、0x43D070 距离+八分码+9 级浮点方向混合表（1.0→0.0125）、0x43CAF0 8 向邻步（dir<4→+4 反向）、**0x43CC30 移动验证器**（dir→邻格→可走→实体占用链 [0x560080]/[0x560070]，类型 [0x88]/状态 [0xC0]/旗 [0x61C74] 门）。
...
## Round 772 (F1078) — 2026-08-13：screen+0xF5200 内嵌地图对象生命周期闭合 + 内嵌容器家族六员全图（F1071 遗留+F1077 次级线索闭合）

- **〔F1071 勘误〕**活图对象非指针——**+0xF5200 是 screen 内嵌成员**（22 个 .text 引用全部 `lea` 取址，唯一 mov 读 @0x41C45B 是首双字 loaded 门测试）；尺寸 ≈0x1B0290（下一成员 +0x2A548C），与 ctor 内 0x6C000 双字(0x1B0000 字节)@+0x1B2 清零 + 8 双字@+0x1B01B4 + 0x20 双字@+0x1B01D4（F1075 计数器组）+0x300 尾区吻合。死对象 0x574118 判定不变。
- **〔ctor/dtor 链〕**screen 构造阶段 7：`lea ecx,[esi+0xF5200]; call 0x43AF70`（wrapper→真入口 **0x43AF90**，F1075 的 0x43B120 系函数中段地址勘误）；析构阶段 7：0x43AF80 `jmp 0x43B190`。ctor 默认值：视口 {+0x16C=0x18,+0x174=0x20,+0x178=0x15,+0x17C=0x28,+0x180=0x1B,+0x184=0x30,+0x188=0x20} + 配置字节 +0x190..0x1B1（4,8,0xC,0x14,0xD6,0xC6,0xAD,0xA5,0x8C,0x5A,0x39,0x63,0xDE,+0xFF 填充）。
- **〔装载路径〕**网络换图处理器 **0x422A92**：ebp=&screen.map、[screen+0x35B25C]=地图Index、锚 [screen+0x2F884C/50]=包内 word、**0x43C9C0**(map, ax−0xC, ay−0xC)=玩家居中滚动锚、**0x43B600**(map, 包内文件名)=装载器；随后 0x38400 字节清 screen+0x154 区。装载器：sprintf 名@+4(fmt@0x47C404)、CreateFileA [0x4760DC] GENERIC_READ、0x43B820 重置、头 0x1C 字节→+0x110、宽 word[+0x126]/高 word[+0x128]、alloc ((w·h)/4)×3 首缓冲——格数据在堆数组，对象内仅静态零区。
...
## Round 832 (全量精读续) — 2026-09-26：物品升级概率系统

> `ObjBase.pas:20600-20820`（`CalcUpgradeProbability`）。**全源码数值最密集的一段**。
> 产物 `items-systems.md` §16。

**〔三段式结果〕**`Result = 2` **성공（成功）** / `1` **불변（不变）** / `0` **파손（破碎）**
→ **「不变」是独立结果**，升级失败不一定毁装备。

**〔属性选项总和 → 概率档（`:20719`）〕**
`iSum := _MIN(10, _MAX(0, SumOfOptions(...)))` → **钳制到 `[0,10]`，11 档**
（注释「옵션합 10 이상은 무시. 옵션합 0이하는 0로 만든다」）。

**〔完整概率表（`iBase = 10000` 定标，已用脚本从源码重算独立校验）〕**
`iValue[0..2]` = **보옥（宝珠）**；`iValue[3..5]` = **신주（神酒）**，
**`신주 = 보옥 × MFactor(4) / DFactor(2) = ×1.5`**。

| 档 | [0]武器 | [1]手镯/鞋 | [2]项链/其他 | 신주[3] | [4] | [5] |
|---:|---:|---:|---:|---:|---:|---:|
...
**概率 = `iValue / 10000`**：档 0 武器 50%、档 9 全部 **0.05%**、
**档 10 恒 0（必失败）**。
**⚠️ 临时改动未回退**（`:20613`）：`MFactor := 4; //보옥의 1.5배로 수정(임시),
원래값 4` —— **注释自相矛盾**（原值就是 4）且 `//임시` **从未移除**
→ **线上跑的是临时加强值**。

**〔装备类型 → 列（`:20739-20750`）〕`StdMode` 值表**：
**`5/6`=武器 → `[0]`**；`10/11`=衣服 → `[0]`；**`24/26/52`=手镯/鞋 → `[1]`**；
**`19`=项链 → `[2]`**；其他 → `[2]`。

**〔幸运值修正（核心公式 `:20741`）〕**
武器：`iSucceed := iValue[0] * ABS((29 + BodyLuckLevel +
  (LOBYTE(AC) + Desc[3] - LOBYTE(MAC) - Desc[4]) / 2) / 30)`
其余：`iSucceed := iValue[k] * ABS((29 + BodyLuckLevel) / 30)`
→ **基准 `29 + BodyLuckLevel`**，**幸运 1 时正好 `30/30 = 1.0` 不修正**。
武器额外项 `(AC + Desc[3] - MAC - Desc[4]) / 2`。
**⚠️ `ABS()` 包住整个比值** → **幸运 <29 时绝对值反而让概率回升**，
**疑为 bug**（本意应是幸运低则概率低，但取绝对值后幸运 0 与 58 修正相同）
—— **已标注未验证**。

**〔攻速特例（`sonmg 2003/12/22`，`:20752-20756`）〕**
`if psJewelryItem.Shape = 9 then iSucceed := (iSucceed * 60) div 100;`
→ **`Shape = 9`（攻速类）成功率打 6 折**，**⏱ 带日期/人名改动**。

**〔两种饰品（`StdMode` 60 vs 61）〕**
**60 = 보옥（宝珠）**：可能破碎（`iFail = (iBase - iSucceed) * 0.7`）；
**61 = 신주（神酒）**：**不破碎**（`iFail` 计算**被注释掉** `:20786`），
只有成功/不变，且用 `iValue[3..5]`（×1.5）
→ **神酒 = 更贵、更安全、成功率更高**。
**⚠️ 又一次临时值未回退**（`:20758`）：
`iFail := Round((iBase - iSucceed) {* 0.65}* 0.7); //임시 0.65로 수정, 원래값 0.7`
—— **注释说临时改 0.65，但代码里 0.65 被注释、实际用 0.7** → **注释与代码相反**。

**〔批量执行与概率回传〕**`iExecCount`（`<1` 置 1）+
`testSucceed`/`testNoChange`/`testFail`（**批量模拟计数**）+
`fRetProb := iSucceed / UpProb[iSum].iBase`（**回传实际概率**）
→ **支持「模拟 N 次」的测试模式**。
