...
| 键位入口 | `window-paint-and-hotkey-dispatch-evidence.json`：裸 E/Ctrl+E→id14；F1-F12 原版技能动作证据与书内绑定仍未闭合 | legacy GameScene 预处理裸 E/Ctrl+E 打开技能书；MagicDialog 选中后处理 F1-F12/Shift+F1-F12；Ctrl/Alt 排除；Ctrl+F1..F4 在开窗时由 GameScene 先切栏组 | 修复 parent/hover 双目标绑定；Ctrl+F1..F4 不再被书内绑定吞掉；当前 Zircon 的普通与 Shift 全量映射已运行验证，但不声称 EI 等价 | 当前构建日志记录 `Spell01..Spell12` 与 `Spell13..Spell24` 全部绑定，截图为 `skill-book-entry-f1-f12-2026-09-25.png`、`skill-book-entry-shift-f1-f12-2026-09-25.png`；Ctrl+F1..F4 路径另有诊断截图 | primary entrance; current runtime complete; EI semantics partial |
